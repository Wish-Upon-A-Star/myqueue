...
You can also upload the current Taskory state directly to AI Board. The sync script logs in, exports JSONL, uploads the new file to `/api/knowledge/upload`, then replaces the previous Taskory knowledge source with the same title after the upload succeeds. It skips repeated uploads when the state file has not changed.
...
By default, the script replaces existing AI Board `taskory` knowledge with the same title after a successful upload, so a long-running watcher does not create duplicate RAG sources and a failed upload does not erase the previous source. Add `--append` only when you intentionally want to keep historical snapshots.
